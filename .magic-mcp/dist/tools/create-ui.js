...
export class CreateUiTool extends BaseTool {
    ...
    async execute({ standaloneRequestQuery, absolutePathToProjectDirectory, message, }) {
        try {
            const server = new CallbackServer();
            const callbackPromise = server.waitForCallback();
            const port = server.getPort();
            let url = `http://21st.dev/magic-chat?q=${encodeURIComponent(standaloneRequestQuery)}&mcp=true&port=${port}`;
            if (config.canvas) {
                ...
                if (config.github) {
                    try {
                        ...
                        const originalBranch = await git(projectDir, "rev-parse --abbrev-ref HEAD");
                        ...
                        await git(projectDir, `commit -m "WIP: magic-ui init" --allow-empty`);
                        ...
                        console.error("Error with git operations, falling back to canvas without branch/repo", error);
                    }
                }
